...
export default function PostgresAdapter(client: Pool): Adapter {
  return {
    ...
    async createUser(user: Omit<AdapterUser, 'id'>) {
      ...
      const sql = `
          INSERT INTO users (first_name, last_name, email, "emailVerified", image) 
          VALUES ($1, $2, $3, $4, $5) 
          RETURNING id, first_name, last_name, email, "emailVerified", image`;
    ...
    async updateUser(user: Partial<AdapterUser>): Promise<AdapterUser> {
      ...
      const updateSql = `
          UPDATE users set
          first_name = $2, last_name = $3, email = $4, "emailVerified" = $5, image = $6
          where id = $1
          RETURNING first_name AS "lastName", last_name AS "lastName", id, email, "emailVerified", image
        `;
    ...
    async getSessionAndUser(sessionToken: string | undefined): Promise<{
      ...
      select
       ...
       emailVerified 
      from users where id = $1`;
      const result2 = await client.query(sql, [session.userId]);
